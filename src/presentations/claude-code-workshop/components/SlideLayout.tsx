...
export function Card({
  ...
  title?: string;
  children?: ReactNode;
  tone?: "plain" | "accent" | "navy" | "good" | "warn";
  className?: string;
  index?: string;
}) {
  const tones: Record<string, string> = {
    plain: "bg-slide-surface border-slide-rule",
    accent: "bg-slide-pink border-slide-pink text-slide-ink",
    navy: "bg-slide-navy-soft border-slide-navy-soft",
    good: "bg-slide-surface border-slide-green/50",
    warn: "bg-slide-surface border-slide-accent/50",
  };
  return (
    <div
      className={cn(
        "flex flex-col border p-[34px]",
        tones[tone],
        className,
      )}
    >
      {index && (
        <span className="slide-kicker mb-[16px] font-mono text-slide-accent">
          {index}
        </span>
      )}
      {title && (
        <h3 className="slide-body-lg font-semibold leading-tight">{title}</h3>
      )}
      {children && (
        <div className="slide-body mt-[16px] text-slide-ink-soft">
          {children}
        </div>
      )}
    </div>
...
export function Rule() {
  return <div className="h-px w-full bg-slide-rule" />;
}
